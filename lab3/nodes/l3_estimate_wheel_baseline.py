...
WHEEL_RADIUS = 0.03303357489713291 #In meters
...
class wheelBaselineEstimator():
    ...
    def startStopCallback(self, msg):
        if self.isMoving is False and np.absolute(msg.angular.z) > 0:
            ...
            del_ang_l = self.del_left_encoder*2*np.pi/TICKS_PER_ROTATION
            del_ang_r = self.del_right_encoder*2*np.pi/TICKS_PER_ROTATION

            separation = (WHEEL_RADIUS/2)*(del_ang_r-del_ang_l)/(NUM_ROTATIONS*2*np.pi)
